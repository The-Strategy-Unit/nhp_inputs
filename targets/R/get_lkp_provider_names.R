get_lkp_provider_names <- function() {
  ...
  dplyr::tbl(con, dbplyr::in_schema("dbo", "DIM_tbOrganisation")) |>
    ...
    dplyr::mutate(
      dplyr::across("organisation_code", stringr::str_sub, 1, 3),
      dplyr::across("organisation_name", stringr::str_to_title),
      dplyr::across("organisation_name", stringr::str_replace, "Nhs", "NHS")
    ) |>
